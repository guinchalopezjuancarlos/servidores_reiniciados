        ...
        <thead>
          <tr className="bg-[#ff7033] text-white text-[10px] uppercase font-bold">
            <th className="py-2 border-x border-orange-400 relative">
              Servidor <button onClick={(e) => { e.stopPropagation(); setMenuAbierto(menuAbierto === 'nombre' ? null : 'nombre'); }}>▽</button>
            ...
            <th className="py-2 border-x border-orange-400 relative">
              Agrupador <button onClick={(e) => { e.stopPropagation(); setMenuAbierto(menuAbierto === 'agrupador' ? null : 'agrupador'); }}>▽</button>
              {menuAbierto === 'agrupador' && <MenuFiltro col="agrupador" datos={datos} filtros={filtros} setFiltros={setFiltros} />}
            </th>
            <th className="py-2 border-x border-orange-400 relative">
              Tipo <button onClick={(e) => { e.stopPropagation(); setMenuAbierto(menuAbierto === 'tipo' ? null : 'tipo'); }}>▽</button>
              {menuAbierto === 'tipo' && <MenuFiltro col="tipo" datos={datos} filtros={filtros} setFiltros={setFiltros} />}
            </th>
            <th className="py-2 border-x border-orange-400 relative">
              Proyecto <button onClick={(e) => { e.stopPropagation(); setMenuAbierto(menuAbierto === 'proyecto' ? null : 'proyecto'); }}>▽</button>
              {menuAbierto === 'proyecto' && <MenuFiltro col="proyecto" datos={datos} filtros={filtros} setFiltros={setFiltros} />}
            </th>
            <th className="py-2 border-x border-orange-400 relative">
              Empresa <button onClick={(e) => { e.stopPropagation(); setMenuAbierto(menuAbierto === 'empresa' ? null : 'empresa'); }}>▽</button>
        ...
        <tbody className="text-[11px] text-slate-700">
          {datosVisibles.length > 0 ? (
            datosVisibles.map((srv) => (
              <tr key={srv.id} className="hover:bg-slate-50 border-b border-slate-200">
                ...
                <td className="p-1 border-x">
                  <button onClick={() => onBorrar(srv.id)} className="bg-red-600 text-white px-2 py-0.5 rounded text-[9px] uppercase font-bold hover:bg-red-700">Quitar</button>
                </td>
              </tr>
            ))
          ) : (
